...
def gen_cli_args():
    ...
    generic_group.add_argument(
        "--version", action="store_true", help="Display nxc version"
    )
    generic_group.add_argument(
        "-t",
        "--threads",
        ...
        dest="threads",
        default=256,
        help="set how many concurrent threads to use",
    )
    generic_group.add_argument(
        "--timeout",
        default=None,
        type=int,
        help="max timeout in seconds of each thread",
    )
    generic_group.add_argument(
        "--jitter",
        metavar="INTERVAL",
        type=str,
        help="sets a random delay between each authentication",
    )
    ...
    output_group.add_argument(
        "--no-progress",
        action="store_true",
        help="do not displaying progress bar during scan",
    )
    output_group.add_argument(
    ...
    dns_group.add_argument(
        "-6", dest="force_ipv6", action="store_true", help="Enable force IPv6"
    )
    dns_group.add_argument(
        "--dns-server",
        action="store",
        help="Specify DNS server (default: Use hosts file & System DNS)",
    )
    dns_group.add_argument(
   ...
   ˙ \   / ˙
    ...
    std_parser.add_argument(
        ...
        nargs="+"
        if not (
            module_parser.parse_known_args()[0].list_modules is not None
            or module_parser.parse_known_args()[0].show_module_options
            or generic_parser.parse_known_args()[0].version
        )
        else "*",
        type=str,
    ...
    certificate_group.add_argument(
        "--pfx-cert",
        metavar="PFXCERT",
        help="Use certificate authentication from pfx file .pfx",
    )
    certificate_group.add_argument(
        "--pfx-base64",
        metavar="PFXB64",
        help="Use certificate authentication from pfx file encoded in base64",
    )
    certificate_group.add_argument(
        "--pfx-pass", metavar="PFXPASS", help="Password of the pfx certificate"
    )
    certificate_group.add_argument(
        "--pem-cert",
        metavar="PEMCERT",
        help="Use certificate authentication from PEM file",
    )
    certificate_group.add_argument(
    ...
    try:
        for protocol in protocols:
            ...
            subparsers = protocol_object.proto_args(
                subparsers, [std_parser, module_parser]
            )
    except Exception as e:
        nxc_logger.exception(
            f"Error loading proto_args from proto_args.py file in protocol folder: {protocol} - {e}"
        )

    # Add dashboard subparser
    from nxc.dashboard.proto_args import add_dashboard_parser
